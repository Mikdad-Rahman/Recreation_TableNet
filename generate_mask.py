...
if __name__ == "__main__":
    directory = './dataset/Marmot_data/'  # Correct directory path
    ...
    for file in os.listdir(directory):
        ...
        if filename.endswith(".xml"):
            ...
            xml_path = os.path.join(directory, filename + '.xml')
            if not os.path.exists(xml_path):
                print(f"[Warning] Skipping: {xml_path} not found.")
                continue

            # Parse xml file
            tree = ET.parse(xml_path)  # Updated to use correct path
            root = tree.getroot()
            size = root.find('size')
